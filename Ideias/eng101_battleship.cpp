/*************************************************************************
Eng 101, Sec 200
Assignment 6b: Battleship -  multiple boats
James Glettler (c) 12 Mar 2001
filename: battleship.cpp
v2.5b
Purpose: Advanved Battleship program.  Single sized ships
         in horiz, vert, or diag line.
Notes:   This program is capable of more than one ship.  No advanced intel.
         on placement of ships or auto-bombing.  However, it does have
         command line arguments that can be used.  Also, the size of the
         board can be increased up to 20x20 by changing the global const
         'SIZE' and the max number of ships will scale correctly.  This
         program runs with ships of length 4 as of this running
         This program traps for crossed ships and eliminates them by special
         case.
         Also: bombing coordinates must be entered in a specific format:
           col number(s),row letter: and must be seperated only by a comma.
Future
Options: - psuedo-animated winning output
         - Non-random auto-bomb
         - Determine size of board at run-time
*************************************************************************/
...
/*-main fn:
/*
/*+fillBoard fn: stores specified char to all elements of the board
/*
/*+dispBoard fn: displays the entire board along with index and key
/*
/*+placeShip fn: places a ship in a random spot without crossing
/*
/*+boardShipRewrite fn: rewrites diagonality info on board to ships
/*
/*+dropBomb fn: inputs user selection for bomb, check validity, checks for ship,
/*updates board (2nd prog will update bombBoard and call disp)
/*
/*+rules fn: print the rules
/*
/*+helpFn fn: prints command line help and info
/*
/*+scoreKeeper fn: keep score using file I/O (command line switch)
/*
/*
/*/
...
{ //Print the rules
   ...
   cout << "The board is created by a " << SIZE << " by " << SIZE << "grid." << endl
        << "The columns of the grid are numbered and the rows are lettered." << endl
        << "The user must enter the desired target as a coordinate pair in the" << endl
        << "form \"#,a\" with a comma seperating the number and the letter." << endl
        << "Or enter \"?\" for the coord. for a random guess by the computer" << endl;
   if (SIZE > 10)
   {
...
{
   cout << "Welcome to Battleship by James Glettler (c) 2001" << endl
        << "This program was written for Engr 100 Sec 300, assignment 6" << endl
        << "This program is capable of up to a 20X20 board based on display "
        << "limits, can" << endl
        << "place as many ships as the given board can "
        << "support and uses two char arrays," << endl
...
int main(int argc, char *argv[])
{
   ...
   if (argc != 1) //if the file is started with command line arguments
   {
      ...
      for (int i = 1; i < argc && i < 6; i = i + 1)
      {
         ...
         if (arg == "rand")
         {
            cout << "Random Time Seed initiallized at " << time(NULL) << " seconds after epoch" << endl;
            srand(time(NULL));
         }
         else if (arg == "help")
         {
   ...
   if (boardDispOption)
      ...
      score = ships * 10 + (totalBombs - bombsDropped) * 8 + 25;
